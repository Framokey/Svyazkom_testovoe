...
<template>
    ...
    <div class="content-header" v-if="!loading">
        <div class="container-fluid">
            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">DetailInfo</h1>
                    <h2>{{ new Date(period['end_date']).toLocaleString('default', { month: 'long', year: 'numeric' }) }}</h2>
                    <h2>Тариф: {{ tariff['tariff'] }}₽ м<sup>3</sup></h2>
                    <h2>Показания счётчика: {{ record['amount_volume'] }} м<sup>3</sup></h2>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item active">DetailInfo</li>
                    </ol>
                </div>
            </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tr v-for="(data, index) in billsData">
                            <th>{{ index + 1 }}</th>
                            <th>{{ new Date(data['resident']['start_date']).toLocaleString('default', { month: 'long', year: 'numeric' }) }}</th>
